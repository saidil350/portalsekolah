...
export default function ClassDetailPage({ params }: { params: Promise<{ classId: string }> }) {
  ...
  return (
    <main className="flex-1 flex flex-col h-full bg-linear-to-br from-slate-50/50 via-white to-blue-50/20">
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200/60 px-6 py-5 shrink-0 sticky top-0 z-10 shadow-sm">
        {/* Breadcrumb */}
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex h-8 items-center gap-2 px-3 bg-primary text-white rounded-md text-xs font-bold hover:bg-primary-600 hover:scale-[1.02] active:scale-[0.98] transition-all"
            >
      ...
      <div className="flex-1 overflow-hidden">
        <div className="flex h-full">
          ...
          <div className="w-2/5 border-r border-border bg-card overflow-y-auto">
            ...
            {/* Statistics */}
            <div className="p-5 border-b border-border/60">
              <h3 className="text-sm font-bold text-foreground mb-3">Statistik</h3>
              ...
              <div className="space-y-2">
                {students.length === 0 ? (
                  ...
                  students.map((student: any) => (
                    <div
                      key={student.id}
                      className="flex items-center justify-between p-2.5 bg-muted/50 rounded-lg hover:bg-accent transition-colors group"
                    >
                      <div className="flex items-center gap-3">
          ...
          <div className="w-3/5 bg-white/40 backdrop-blur-sm overflow-y-auto">
            <div className="p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="flex h-8 items-center gap-2 px-3 bg-blue-600 text-white rounded-md text-xs font-bold hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] transition-all"
                >
              ...
              {/* Schedule Grid */}
              <div className="bg-card rounded-xl border border-border shadow-md shadow-slate-200/40 overflow-hidden">
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-slate-50/80 border-b border-border">
                      <th className="w-20 px-4 py-3 text-left font-bold text-muted-foreground text-[10px] uppercase tracking-wider cursor-default">Jam</th>
                      {days.map(day => (
                        <th key={day.value} className="px-4 py-3 text-center font-bold text-foreground text-[11px] uppercase tracking-wider cursor-default">
                          {day.label}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {timeSlots.map((time, timeIdx) => (
                      <tr key={time} className="hover:bg-slate-50/30 transition-colors">
                        <td className="px-4 py-3 text-muted-foreground text-xs font-bold align-top whitespace-nowrap bg-slate-50/20">
                          {time}
                        </td>
                        {days.map(day => {
                          ...
                          return (
                            <td key={day.value} className="px-2 py-2 align-top h-24">
                              {schedule ? (
                                <div className="h-full bg-linear-to-br from-blue-50 to-indigo-50/50 border border-blue-100 rounded-xl p-2.5 transition-all hover:shadow-lg hover:shadow-blue-200/40 hover:-translate-y-0.5 group cursor-pointer ring-1 ring-blue-500/0 hover:ring-blue-500/30">
            ...
            {/* Teachers List */}
            <div className="mt-2 p-5 border-t border-border/60 bg-linear-to-b from-transparent to-slate-50/30">
              <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
          ...
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-5 flex items-center justify-end gap-2.5">
              <button
                ...
                className="h-8 px-3 bg-muted text-foreground rounded-md text-xs hover:bg-slate-200 transition-colors disabled:opacity-50"
              >
                Batal
              </button>
              <button
                ...
                disabled={withdrawing}
                className="h-8 px-3 bg-red-600 text-white rounded-md text-xs hover:bg-red-700 transition-colors disabled:opacity-50"
              >
