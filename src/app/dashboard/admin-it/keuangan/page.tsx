          ...
          <div className="max-w-[1280px] flex flex-col gap-6 mx-auto w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full">
              <div className="bg-card border border-border rounded-xl p-4 relative shadow-sm hover:shadow-md transition-shadow">
                ...
                <h3 className="text-foreground text-2xl font-bold mb-2">Rp 562.400.000</h3>
              ...
              <div className="bg-card border border-border rounded-xl p-4 relative shadow-sm hover:shadow-md transition-shadow">
                ...
                <h3 className="text-foreground text-2xl font-bold mb-2">Rp 124.500.000</h3>
              ...
              <div className="bg-card border border-border rounded-xl p-4 relative shadow-sm hover:shadow-md transition-shadow">
                ...
                <h3 className="text-foreground text-2xl font-bold mb-2">Nov 1, 2023</h3>
                <p className="text-muted-foreground text-xs font-medium">{t('admin.finance.card.nextCycle.desc')}</p>
              </div>
            </div>
                  ...
                  <div className="p-5 flex flex-col gap-4">
                    ...
                    <div className="border border-border/60 rounded-xl p-3.5 flex flex-col sm:flex-row gap-3 sm:items-center w-full hover:border-slate-200 transition-colors">
                      ...
                      <div className="flex-1 flex flex-col">
                        <span className="text-foreground text-base font-semibold">{t('admin.finance.rates.gradeX')}</span>
                        <span className="text-muted-foreground text-sm">{t('admin.finance.rates.streams.scienceSocial')}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          ...
                          <input type="text" value={rates.grade10} onChange={(e) => setRates(p => ({ ...p, grade10: e.target.value }))} title="Tuition Rate Grade X" placeholder="450.000" aria-label="Tuition Rate Grade X" className="bg-muted/50 border border-border text-foreground text-sm font-semibold rounded-lg focus:ring-2 focus:ring-primary/30 focus:border-primary block w-[160px] pl-10 pr-4 py-2 outline-none transition-all" />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border border-border/60 rounded-xl p-3.5 flex flex-col sm:flex-row gap-3 sm:items-center w-full hover:border-slate-200 transition-colors">
                      ...
                      <div className="flex-1 flex flex-col">
                        <span className="text-foreground text-base font-semibold">{t('admin.finance.rates.gradeXI')}</span>
                        <span className="text-muted-foreground text-sm">{t('admin.finance.rates.streams.scienceSocial')}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          ...
                          <input type="text" value={rates.grade11} onChange={(e) => setRates(p => ({ ...p, grade11: e.target.value }))} title="Tuition Rate Grade XI" placeholder="475.000" aria-label="Tuition Rate Grade XI" className="bg-muted/50 border border-border text-foreground text-sm font-semibold rounded-lg focus:ring-2 focus:ring-primary/30 focus:border-primary block w-[160px] pl-10 pr-4 py-2 outline-none transition-all" />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border border-border/60 rounded-xl p-3.5 flex flex-col sm:flex-row gap-3 sm:items-center w-full hover:border-slate-200 transition-colors">
                      ...
                      <div className="flex-1 flex flex-col">
                        <span className="text-foreground text-base font-semibold">{t('admin.finance.rates.gradeXII')}</span>
                        <span className="text-muted-foreground text-sm">{t('admin.finance.rates.streams.all')}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                          ...
                          <input type="text" value={rates.grade12} onChange={(e) => setRates(p => ({ ...p, grade12: e.target.value }))} title="Tuition Rate Grade XII" placeholder="500.000" aria-label="Tuition Rate Grade XII" className="bg-muted/50 border border-border text-foreground text-sm font-semibold rounded-lg focus:ring-2 focus:ring-primary/30 focus:border-primary block w-[160px] pl-10 pr-4 py-2 outline-none transition-all" />
                        </div>
                      </div>
                    </div>
              ...
              <div className="w-full lg:w-[320px] flex flex-col gap-5 shrink-0">
                ...
                <div className="bg-card border border-border rounded-xl shadow-sm p-5 w-full flex flex-col gap-3.5">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className={`border rounded-lg p-3 flex items-center justify-between transition-all duration-200 ${virtualAccount ? 'bg-muted/50 border-border/60' : 'bg-card border-border opacity-60'}`}>
                      <div className="flex gap-3 items-center">
                        <div className="w-10 h-10 bg-card border border-border rounded flex items-center justify-center shrink-0">
                          <Landmark className={`w-5 h-5 transition-colors ${virtualAccount ? 'text-blue-600' : 'text-muted-foreground'}`} />
                        </div>
                        <div className="flex flex-col">
                          <span className="text-foreground text-sm font-semibold">{t('admin.finance.payment.method.va')}</span>
                          <span className={`text-[10px] font-medium uppercase mt-0.5 transition-colors ${virtualAccount ? 'text-emerald-600' : 'text-muted-foreground'}`}>{virtualAccount ? t('admin.finance.payment.method.va.desc.active') : t('admin.finance.payment.method.disabled')}</span>
                        </div>
                      </div>
                    ...
                    {/* Method 2 - Cash */}
                    <div className={`border rounded-lg p-3 flex items-center justify-between transition-all duration-200 ${cashManual ? 'bg-muted/50 border-border/60' : 'bg-card border-border opacity-60'}`}>
                      <div className="flex gap-3 items-center">
                        <div className="w-10 h-10 bg-card border border-border rounded flex items-center justify-center shrink-0">
                          <HandCoins className={`w-5 h-5 transition-colors ${cashManual ? 'text-foreground' : 'text-muted-foreground'}`} />
                        </div>
                        <div className="flex flex-col">
                          <span className="text-foreground text-sm font-semibold">{t('admin.finance.payment.method.cash')}</span>
                          <span className={`text-[10px] font-medium uppercase mt-0.5 transition-colors ${cashManual ? 'text-amber-600' : 'text-muted-foreground'}`}>{cashManual ? t('admin.finance.payment.method.cash.desc.active') : t('admin.finance.payment.method.disabled')}</span>
                        </div>
                      </div>
                    ...
                    {/* Method 3 - QRIS */}
                    <div className={`border rounded-lg p-3 flex items-center justify-between transition-all duration-200 ${qrisPayment ? 'bg-muted/50 border-border/60' : 'bg-card border-border opacity-60'}`}>
                      <div className="flex gap-3 items-center">
                        <div className="w-10 h-10 bg-card border border-border rounded flex items-center justify-center shrink-0">
                          <QrCode className={`w-5 h-5 transition-colors ${qrisPayment ? 'text-purple-600' : 'text-muted-foreground'}`} />
                        </div>
                        <div className="flex flex-col">
                          <span className="text-foreground text-sm font-semibold">{t('admin.finance.payment.method.qris')}</span>
                          <span className={`text-[10px] font-medium uppercase mt-0.5 transition-colors ${qrisPayment ? 'text-emerald-600' : 'text-muted-foreground'}`}>{qrisPayment ? t('admin.finance.payment.method.active') : t('admin.finance.payment.method.disabled')}</span>
                        </div>
                      </div>
                ...
                <div className="bg-card border border-border rounded-xl shadow-sm p-5 w-full flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-3 mt-2">
                     <label className="flex items-start gap-3 cursor-pointer group" onClick={() => setLatePenalty(!latePenalty)}>
                        <div className={`w-5 h-5 border rounded flex items-center justify-center shrink-0 mt-0.5 transition-all duration-200 ${latePenalty ? 'bg-primary border-primary' : 'bg-card border-slate-300'}`}>
                            {latePenalty && (
                     ...
                     <div className={`relative mt-1 transition-all duration-200 ${latePenalty ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                        ...
                        <input type="text" value={penaltyAmount} onChange={(e) => setPenaltyAmount(e.target.value)} title="Late Payment Penalty" placeholder="25.000" aria-label="Late Payment Penalty" className="bg-muted border-none text-foreground text-sm font-medium rounded-lg block w-full pl-10 pr-4 py-2 outline-none focus:ring-2 focus:ring-primary/30 transition-all" />
                     </div>
                  </div>
                ...
                <div className="w-full">
                  <div className="bg-slate-50/50 grid grid-cols-[1.5fr_1fr_1fr_1.5fr_1fr] border-b border-border/60">
                    {[
                      ...
                      <div key={h} className={`px-4 py-3 ${i === 4 ? 'text-right' : ''}`}>
                        <span className="text-muted-foreground text-xs font-bold uppercase tracking-wider whitespace-nowrap">{h}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {incompleteStudents
                    ...
                    <div key={index} className="grid grid-cols-[1.5fr_1fr_1fr_1.5fr_1fr] border-b border-slate-50 items-center hover:bg-slate-50/50 transition-colors">
                      ...
                      <div className="px-4 py-3"><span className="text-muted-foreground text-sm whitespace-nowrap">{student.lastPayment}</span></div>
