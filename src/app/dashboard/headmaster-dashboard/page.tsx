      ...
      <div className="flex-1 overflow-y-auto p-12">
        <div className="max-w-[1280px] mx-auto flex flex-col gap-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
              <h2 className="text-foreground text-4xl font-bold tracking-tight">{t('headmaster.db.title')}</h2>
              <p className="text-muted-foreground text-base mt-2">
                {t('headmaster.db.subtitle')}
              </p>
            </div>
            <div className="flex items-center gap-2 px-3.5 py-2.5 bg-card border border-border rounded-lg shadow-sm">
              <Calendar className="w-4 h-4 text-muted-foreground" />
              <span className="text-sm font-medium text-muted-foreground">{t('headmaster.db.semesterLabel')}</span>
            </div>
          </div>
            ...
            {kpiCards.map((kpi) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-3xl font-bold text-foreground leading-tight">{kpi.value}</h3>
          ...
          <div className="bg-card rounded-xl border border-border shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-bold text-foreground">{t('headmaster.db.cashflowTitle')}</h3>
                <p className="text-sm text-muted-foreground mt-0.5">{t('headmaster.db.cashflowDesc')}</p>
              </div>
              ...
              <div className="absolute inset-0 flex flex-col justify-between pb-8">
                {['500M', '400M', '300M', '200M', '0'].map((label, i) => (
                  <div key={i} className="flex items-center w-full">
                    <span className="text-xs text-muted-foreground w-10 text-right mr-3 shrink-0">{label}</span>
              ...
              <div className="absolute inset-0 flex items-end justify-around pl-14 pb-8 gap-2">
                {barChartData.map((bar) => (
                  <div key={bar.month} className="flex flex-col items-center flex-1 max-w-[64px]">
                    ...
                    <span className={`text-xs mt-2 ${bar.active ? 'font-bold text-foreground' : 'font-medium text-muted-foreground'}`}>
                      {bar.month}
                    </span>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-8">
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm p-6">
              <div className="mb-6">
                <h3 className="text-lg font-bold text-foreground">{t('headmaster.db.avgGradesTitle')}</h3>
                <p className="text-sm text-muted-foreground mt-0.5">{t('headmaster.db.avgGradesDesc')}</p>
              </div>
              ...
              <div className="flex items-center gap-2 mt-6 pt-4 border-t border-border/60">
                ...
                <span className="text-sm text-muted-foreground">{t('headmaster.db.averageScore')}</span>
              </div>
            </div>
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm p-6">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h3 className="text-lg font-bold text-foreground">{t('headmaster.db.attendanceTitle')}</h3>
                  <p className="text-sm text-muted-foreground mt-0.5">{t('headmaster.db.attendanceDesc')}</p>
                </div>
              ...
              <div className="flex items-center justify-center gap-8">
                ...
                <div className="relative w-48 h-48 shrink-0">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('headmaster.db.status.present')}</span>
                  </div>
                </div>
