...
const SETTINGS_PATH = '/dashboard/student-dashboard/settings'
...
export async function updateStudentProfile(formData: FormData) {
  ...
  const notificationPreferences = {
    ...
    tuition: formData.get('tuitionNotifications') === 'on',
...
export async function updateNotificationPreferences(email: boolean, tuition: boolean, push: boolean) {
  const auth = await authorizeAction(['SISWA'])
  if (!auth.success) {
    return { success: false, error: auth.error }
  }
  ...
  try {
    const { error } = await supabase
      .from('profiles')
      .update({
        notification_preferences: { email, tuition, push },
        updated_at: new Date().toISOString(),
      })
      .eq('id', user.id)
