        ...
        <div className="max-w-[1280px] p-6 flex flex-col gap-5">
          ...
          <div className="flex items-end justify-between">
            <div className="flex flex-col gap-1">
              ...
              <h2 className="text-2xl font-bold text-foreground tracking-tight">{t('student.grades.title')}</h2>
              <p className="text-sm text-muted-foreground">{t('student.grades.subtitle')}</p>
            </div>
            ...
            <div className="bg-card border border-border rounded-xl shadow-sm p-2 flex items-center gap-2">
              ...
              <button title="Download" className="bg-primary text-white p-2 rounded-lg hover:bg-primary/90 transition-colors cursor-pointer ml-1">
          ...
          <div className="bg-card rounded-xl border border-border shadow-sm p-5">
            ...
            {/* Bar Chart */}
            <div className="h-[220px] flex items-end justify-between px-3 relative">
              {/* Grid lines */}
              <div className="absolute inset-0 flex flex-col justify-between pb-7 pt-1 pointer-events-none">
              ...
              {chartData.map((d, i) => (
                <div key={i} className="flex flex-col items-center gap-2 flex-1 relative z-10">
                  <div className="flex items-end h-[170px]">
                    <div
                      className="w-8 bg-linear-to-t from-primary to-blue-400 rounded-t-lg shadow-[0px_0px_15px_0px_rgba(19,127,236,0.2)] transition-all duration-500 hover:opacity-80"
                      style={{ height: `${(d.score / 100) * 170}px` } as React.CSSProperties}
                    />
                  </div>
          ...
          <div className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              ...
              <button className="bg-card border border-border rounded-lg px-3 py-1.5 flex items-center gap-2 text-xs font-medium text-muted-foreground hover:bg-accent transition-colors cursor-pointer">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-muted/50">
                  <th className="text-left pl-5 pr-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('student.grades.table.subject')}</th>
                  <th className="text-center px-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('student.grades.table.credits')}</th>
                  <th className="text-center px-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('student.grades.table.midterm')}</th>
                  <th className="text-center px-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('student.grades.table.final')}</th>
                  <th className="text-center px-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('student.grades.table.grade')}</th>
                  <th className="text-right pr-5 pl-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide">{t('student.grades.table.point')}</th>
                </tr>
              </thead>
              <tbody>
                {subjectData.map((item, i) => (
                  <tr key={i} className="border-t border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="text-center px-3 py-3.5 text-sm font-medium text-foreground">{item.credits}</td>
                    <td className="text-center px-3 py-3.5 text-sm font-medium text-foreground">{item.midterm}</td>
                    <td className="text-center px-3 py-3.5 text-sm font-medium text-foreground">{item.final}</td>
                    ...
                    <td className="text-right pr-5 pl-3 py-3.5 text-sm font-semibold text-foreground">{item.point}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-slate-50/80">
                  <td className="pl-5 pr-3 py-5 text-sm font-semibold text-foreground">{t('student.grades.summary.total')}</td>
                  <td className="text-center px-3 py-5 text-sm font-semibold text-foreground">18</td>
                  <td colSpan={3}></td>
                  <td className="text-right pr-6 pl-4 py-4">
                    <p className="text-xl font-bold text-primary">3.58</p>
