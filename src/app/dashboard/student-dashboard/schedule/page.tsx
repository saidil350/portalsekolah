      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-border px-6 py-4 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <button title="Notifications" className="relative p-2 rounded-full hover:bg-accent transition-colors cursor-pointer">
            <Bell className="w-4 h-5 text-muted-foreground" />
            ...
            <div className="flex-1 bg-card rounded-xl border border-border shadow-sm p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">{t('student.performance.chart.gpa')}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-slate-300" />
                    <span className="text-sm text-muted-foreground">{t('student.performance.chart.avg')}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="relative h-[280px] border-b border-border">
                ...
                <div className="ml-10 mr-4 flex justify-between mt-2">
                  {trendData.map((d) => (
                    <span key={d.bulan} className="text-xs font-medium text-muted-foreground flex-1 text-center">{d.bulan}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            {teacherComments.map((item, i) => (
              <div key={i} className="bg-card rounded-xl border border-border shadow-sm p-6">
                ...
                <div className="bg-muted/50 rounded-lg p-4 relative">
                  ...
                  <p className="text-sm text-muted-foreground leading-5 pl-8 mb-4">
                    {item.comment}
                  </p>
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <span className="text-xs font-semibold text-foreground">{item.teacher}</span>
                  </div>
                </div>
              </div>
            ))}
